...
class CareerController extends DM_BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        //     'description' => 'sometimes|500',
        //     'status' => 'required|boolean'
        // ]);

        if ($this->model->storeData($request, $request->title, $request->category, $request->level, $request->experience, $request->number, $request->time , $request->salary, $request->apply_before,$request->status)) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($this->model->updateData($request, $id, $request->name, $request->url, $request->image, $request->description, $request->status)) {
