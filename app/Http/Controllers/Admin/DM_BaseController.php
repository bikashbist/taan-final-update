...
class DM_BaseController extends Controller
{
    ...
    {
       // dd($folder_path_image, $prefix_path_image, $title, $image_width, $image_height);
     ...
     */
    protected function uploadSingleFile(Request $request, $folder_path_file, $prefix_path_file, $name)
    {
        if ($request->hasFile($name)) {
            $this->createFolder($folder_path_file);
            $file = $request->file($name);
            ...
            //    $file_extension = $file->getClientOriginalExtension();
            $file->move($folder_path_file, $file_name);
            $file_path = $prefix_path_file . $file_name;
            return $file_path;
        }
        return false;
    }
